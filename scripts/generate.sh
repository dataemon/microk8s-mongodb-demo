...
microk8s kubectl create secret generic shared-bootstrap-data --from-file=internal-auth-mongodb-keyfile=$TMPFILE
rm $TMPFILE
...
microk8s kubectl apply -f ../resources/mongodb-service.yaml --validate=false
sleep 5
